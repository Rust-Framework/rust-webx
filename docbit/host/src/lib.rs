...
// Registers the `wwwroot` table generated by build.rs, so `build_host()` can
// enable it with `.embed()` and the deployed binary needs no wwwroot/ at all.
...
pub fn build_host() -> Host {
    ...
    let mut builder = Host::builder()
        ...
        .add_memory_cache()
        // Static files ship inside the executable; `wwwroot/` beside the binary
        // overrides individual files such as a custom logo or favicon.
        .use_spa("wwwroot")
        .embed();
