...
use webx_core::auth::{IAuthorizationPolicy, IClaims};
...
async fn authz_user_with_allowed_role_is_authorized() {
    let policy = webx_host::authz::ResourceAuthorization::new().allow_role("/api/admin", "admin");
...
async fn authz_user_with_permission_is_authorized() {
    let policy = webx_host::authz::ResourceAuthorization::new()
