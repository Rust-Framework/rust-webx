...
rust-webx 框架当前呈现"定义齐备、实现参差"的特征。三大跨层架构债正在系统性侵蚀设计意图：
...
2. **IPipelineBehavior 空壳 + Mediator::send 用 root provider**：trait 已定义但 [mediator/pipeline.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/mediator/pipeline.rs) 是空壳，`Mediator::send` 全程未串联行为链；且用 root provider 解析 scoped 服务会降级为 transient，与 HTTP 路径行为不一致。
...
1. 删除整文件：
   ...
   * [crates/macros/src/controller.rs](file:///e:/GitCode/RF/rust-webx/crates/macros/src/controller.rs)

   * [crates/macros/src/route.rs](file:///e:/GitCode/RF/rust-webx/crates/macros/src/route.rs)

2. 修改 [crates/macros/src/endpoint.rs](file:///e:/GitCode/RF/rust-webx/crates/macros/src/endpoint.rs)：删除 `request_macro_impl`（L47-53）

3. 修改 [crates/macros/src/lib.rs](file:///e:/GitCode/RF/rust-webx/crates/macros/src/lib.rs)：移除 `controller`、`http_get`、`http_post`、`http_put`、`http_delete`、`request` 六个宏的导出（L24-27、L114-135、L223-225）。同时移除 `mod controller;`、`mod route;`

4. 修改 [crates/webx/src/lib.rs](file:///e:/GitCode/RF/rust-webx/crates/webx/src/lib.rs#L72-75)：从 `pub use rust_webx_macros::{...}` 中移除 `controller, http_delete, http_get, http_post, http_put, request`

5. 修改 [crates/macros/Cargo.toml](file:///e:/GitCode/RF/rust-webx/crates/macros/Cargo.toml)：description 改为 `rust-webx macros: route shortcuts + handler auto-registration`（去掉"控制器"字样）

6. 修改 [crates/core/src/route/scan.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/route/scan.rs)：L7、L47、L87 doc comment 中的 `#[controller]` 字样删除或改为 `#[get/post/...]`

7. 文档同步（仅删除虚假宣传）：

   * [README.md](file:///e:/GitCode/RF/rust-webx/README.md) L86、L92、L163 删除 `crud_controller` 示例引用、"控制器"字样

   * `docs/rust-webx/05-request-pattern/route-macros.md`、`docs/rust-webx/01-introduction/ecosystem-overview.md`、`docs/rust-webx/13-extensibility/custom-endpoints.md` 删除 controller 范式段落（不重写，只删除）
...
**改动文件**：[crates/core/src/mediator/default.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/mediator/default.rs)
...
**新增测试** [crates/host/tests/mediator\_test.rs](file:///e:/GitCode/RF/rust-webx/crates/host/tests/mediator_test.rs)：注册一个带状态计数器的 Scoped handler，连续两次 `Mediator::send` 验证每次获得新实例（计数器不累加）。

**验证**：`cargo test -p rust-webx-host mediator_test` 通过。

***

### 阶段 3：P0-3 IPipelineBehavior 链实现

**改动文件**：[crates/core/src/mediator/pipeline.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/mediator/pipeline.rs)（当前空壳）
...
**改动文件**：[crates/core/src/mediator/default.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/mediator/default.rs) `send` 方法
...
注意：`entry.call` 签名需对照 [crates/core/src/route/scan.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/route/scan.rs) 中的 `HandlerRegistration` 确认（factory 返回 `Box<dyn Any>`，call 接收 handler + req 返回 `Result<Box<dyn Any + Send>>`）。

**保持不变**：

* `IPipelineBehavior` trait 签名（[crates/core/src/pipeline.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/pipeline.rs) L35-42，用户明确要求不可改 trait）

* `add_pipeline::<T>()` 注册入口（singleton + `Arc::new(T::default())`）

* `BoxedNextFn` / `BoxedPipelineFuture` 类型别名

**新增测试** [crates/host/tests/mediator\_test.rs](file:///e:/GitCode/RF/rust-webx/crates/host/tests/mediator_test.rs)：实现 `LoggingBehavior` + `ValidationBehavior` 两个测试 behavior，验证：

1. 调用顺序：behavior1 → behavior2 → handler → 反向回到 behavior2 → behavior1
2. behavior 可以短路（不调 next 直接返回）
3. 无 behavior 注册时直接走 handler（向后兼容）

**验证**：`cargo test -p rust-webx-host mediator_test` 全部通过；现有 send/publish 测试不破坏。
...
**改动文件**：[crates/core/src/middleware.rs](file:///e:/GitCode/RF/rust-webx/crates/core/src/middleware.rs) L15-30
...
**改动文件**：[crates/host/src/pipeline.rs](file:///e:/GitCode/RF/rust-webx/crates/host/src/pipeline.rs) L39-58
...
| 中间件                         | 文件                                                                                                         | 改动                                        |
| --------------------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `AuthMiddleware`            | [auth\_jwt.rs:181-189](file:///e:/GitCode/RF/rust-webx/crates/host/src/auth_jwt.rs#L181-189)             | invoke 末尾 `Ok(ControlFlow::Continue(()))` |
| `ResourceAuthMiddleware`    | [authz.rs:151-173](file:///e:/GitCode/RF/rust-webx/crates/host/src/authz.rs#L151-173)                    | `Err` 保持；成功路径返回 `Continue`                |
| `CorsMiddleware`            | [cors.rs:60-116](file:///e:/GitCode/RF/rust-webx/crates/host/src/cors.rs#L60-116)                        | **OPTIONS 分支返回** **`Break(())`**（核心修复）    |
| `RateLimitMiddleware`       | [rate\_limit.rs:122-142](file:///e:/GitCode/RF/rust-webx/crates/host/src/rate_limit.rs#L122-142)         | **429 分支返回** **`Break(())`**（核心修复）        |
| `RequestIdMiddleware`       | [request\_id.rs:24-29](file:///e:/GitCode/RF/rust-webx/crates/host/src/request_id.rs#L24-29)             | 末尾 `Continue`                             |
| `TimingMiddleware`          | [timing.rs:29-39](file:///e:/GitCode/RF/rust-webx/crates/host/src/timing.rs#L29-39)                      | 末尾 `Continue`                             |
| `RequestTracing`            | [request\_tracing.rs:31-64](file:///e:/GitCode/RF/rust-webx/crates/host/src/request_tracing.rs#L31-64)   | 末尾 `Continue`                             |
| `SecurityHeadersMiddleware` | [security\_headers.rs:26-52](file:///e:/GitCode/RF/rust-webx/crates/host/src/security_headers.rs#L26-52) | 末尾 `Continue`                             |
| `SpaMiddleware`             | [crates/spa/src/spa.rs](file:///e:/GitCode/RF/rust-webx/crates/spa/src/spa.rs)                           | **命中静态文件后返回** **`Break(())`**             |
...
**改动文件**：[crates/host/src/server.rs](file:///e:/GitCode/RF/rust-webx/crates/host/src/server.rs) HostBuilder（L62-211）
...
**改动文件**：[crates/host/tests/pipeline\_test.rs](file:///e:/GitCode/RF/rust-webx/crates/host/tests/pipeline_test.rs)
...
**验证**：`cargo test -p rust-webx-host` 全部通过；`cargo test -p rust-webx-spa` 通过。
...
**改动文件**：[crates/host/src/health.rs](file:///e:/GitCode/RF/rust-webx/crates/host/src/health.rs)
...
**改动文件**：[crates/host/src/server.rs](file:///e:/GitCode/RF/rust-webx/crates/host/src/server.rs)
...
**改动文件**：[crates/host/src/server.rs](file:///e:/GitCode/RF/rust-webx/crates/host/src/server.rs) L402-408
...
**新增测试** [crates/host/tests/health\_test.rs](file:///e:/GitCode/RF/rust-webx/crates/host/tests/health_test.rs)：
...
**验证**：`cargo test -p rust-webx-host health_test` 通过。
...
# 阶段 2 (P0-5)
cargo test -p rust-webx-host mediator_test
...
cargo test -p rust-webx-spa
cargo test -p rust-webx-core
# 验证：所有现有测试 + 新增短路测试

# 阶段 5 (P0-2)
cargo test -p rust-webx-host health_test
