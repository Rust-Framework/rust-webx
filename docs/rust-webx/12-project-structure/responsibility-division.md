...
| domain | 实体、值对象、EF 配置、seed | host 依赖、Handler 逻辑 |
| host/startup | DI 组装（基础设施）、Host 配置、`IHostedService` | 业务逻辑、业务 Service 手动注册 |
...
是否是持久化实体、EF 配置或 seed？             → domain
是否是应用启动/初始化？                        → host/startup
是否是 Host / 缓存 / 认证等框架配置？           → host/src/main.rs + appsettings.json
```
...
- domain **可以引用** contracts（复用枚举、共享 model）与框架核心原语（`rust-webx-core`）
- domain **禁止引用** handlers 或 `host`
- handlers **可以引用** contracts、domain 与基础设施
...
2. `handlers/` — 实现 Service + Handler，`#[inject]` 标注在 `impl Trait for Type` 上
3. `domain/` — 如需持久化，添加实体与 EF 配置
4. `host/src/main.rs` — 通常无需修改
...
- domain 中 `use webx_host::*` → 领域被 host 污染
