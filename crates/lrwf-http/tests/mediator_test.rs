...
use lrwf_core::di::scan::{HandlerCache, ResponseData};
use lrwf_core::error::{Error, Result as LrwfResult};
use lrwf_core::handler::IEventHandler;
use lrwf_core::handler::IRequestHandler;
use lrwf_core::mediator::{IEventRequest, IMediator, IRequest};
use lrwf_core::mediator_impl::Mediator;
use std::any::Any;
...
// ─── Handler (native async fn — no #[async_trait]) ───────────────────
...
fn build_cache(req_type_name: &'static str, handler_type: &str) -> HandlerCache {
    ...
    if handler_type == "hello" {
        ...
        entries.insert(
            req_type_name,
            Arc::new(lrwf_core::di::scan::HandlerEntry {
                handler,
