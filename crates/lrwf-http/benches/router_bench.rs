...
impl IEndpoint for BenchEndpoint {
    async fn handle(&self, _ctx: &mut dyn IHttpContext) -> lrwf_core::error::Result<()> {
        Ok(())
    }
}
...
impl lrwf_core::http::IClaimsExt for TestHttpContext {
    fn set_claims(&mut self, claims: Box<dyn lrwf_core::auth::IClaims>) {
