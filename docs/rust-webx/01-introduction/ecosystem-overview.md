...
rust-webapp 以 Cargo Workspace 组织，各 Crate 职责单一、依赖方向清晰：

```
rust-webapp/                    # Workspace 根
├── crates/
│   ├── core/                   # rust-webapp-core — 核心 trait（零实现依赖）
│   ├── host/                   # rust-webapp-host — Host、管道、路由、认证实现
│   ├── macros/                 # rust-webapp-macros — 过程宏
│   ├── spa/                    # rust-webapp-spa — SPA 静态托管
│   ├── openapi/                # rust-webapp-openapi — OpenAPI 生成
│   └── webapp/                 # rust-webapp — 伞 Crate，统一导出
├── docbit/                     # 示例：作品集全栈应用
└── docs/rust-webapp/           # 本书文档（仓库根目录）
```
...
```mermaid
graph BT
    Webapp[rust-webapp 伞 Crate]
    Core[rust-webapp-core]
    Host[rust-webapp-host]
    Macros[rust-webapp-macros]
    SPA[rust-webapp-spa]
    OpenAPI[rust-webapp-openapi]
...
作为框架使用者，你**通常只需依赖 `rust-webapp` 伞 Crate**。仅在以下情况直接接触子 Crate：

- 编写独立中间件库 → 依赖 `rust-webapp-core`
- 贡献框架本身 → 修改对应子 Crate
...
rust-webapp 的 Crate 分层体现了**接口与实现分离**的设计：core 定义契约，host 提供运行时，macros 提供编译时便利，伞 Crate 统一开发者体验。
