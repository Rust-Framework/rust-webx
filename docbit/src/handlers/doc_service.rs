...
use crate::contracts::docs::{DocContent, DocIndex, DocIndexItem, IDocumentService};
use crate::contracts::work::WorkModel;
...
impl IDocumentService for DocService {
    ...
    fn list_portfolio(&self) -> Result<Vec<WorkModel>, String> {
    ...
    fn get_portfolio(&self, slug: &str) -> Result<WorkModel, String> {
        let index_path = self.work_dir(slug).join("INDEX.json");
        if !index_path.is_file() {
...
impl DocService {
    fn load_portfolio_item(&self, dir_slug: &str) -> Result<WorkModel, String> {
        let raw = fs::read_to_string(self.work_dir(dir_slug).join("INDEX.json"))
            .map_err(|e| e.to_string())?;

        if let Ok(root) = serde_json::from_str::<IndexRootV2>(&raw) {
            return Ok(self.meta_to_work_model(dir_slug, &root.meta, !root.parts.is_empty()));
        }
    ...
    fn meta_to_work_model(&self, dir_slug: &str, meta: &IndexMeta, has_docs: bool) -> WorkModel {
        let slug = meta
            .slug
            .clone()
            .filter(|s| !s.is_empty())
        ...
        let logo_name = meta.logo.as_deref().unwrap_or("logo.svg");
        ...
        WorkModel {
            ...
            logo_url: self.logo_public_url(dir_slug, logo_name),
        }
    }
